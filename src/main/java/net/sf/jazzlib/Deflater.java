...
public class Deflater {
    ...
    /**
     * This strategy will only allow longer string repetitions. It is useful for random data with a small character
     * set.
     */
     ...
     * @param lvl the compression level, a value between NO_COMPRESSION and BEST_COMPRESSION, or DEFAULT_COMPRESSION.
    ...
    /**
     * Creates a new deflater with given compression level.
     * @param lvl    the compression level, a value between NO_COMPRESSION and BEST_COMPRESSION.
     * @param nowrap true, iff we should suppress the deflate header at the beginning and the adler checksum at the end
     *               of the output. This is useful for the GZIP format.
     * @throws IllegalArgumentException if lvl is out of range.
     */
     ...
     * will be thrown away. The given byte array should not be changed, before needsInput() returns true again. This
     * call is equivalent to <code>setInput(input, 0, input.length)</code>.
     ...
     * @throws IndexOutOfBoundsException if offset and/or length don't match the array length.
     */
     ...
     * <code>setDictionary(dict, 0, dict.length)</code>.
     * @param dict the dictionary.
     * @throws IllegalStateException if setInput () or deflate () were already called or another dictionary was already
     *                               set.
     */
