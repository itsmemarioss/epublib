...
public class EpubWriterTest {
    ...
    public void testBook1() throws IOException {
        ...
        Assert.assertEquals(Identifier.Scheme.ISBN,
                CollectionUtil.first(readBook.getMetadata().getIdentifiers()).getScheme());
        Assert.assertEquals(CollectionUtil.first(book.getMetadata().getIdentifiers()).getValue(),
    ...
    /**
     * Test for a very old bug where epublib would throw a NullPointerException when writing a book with a cover that
     * has no id.
     * @throws IOException
    ...
    private Book createTestBook() throws IOException {
        ...
        TOCReference chapter2 = book.addSection("Second chapter",
                new Resource(this.getClass().getResourceAsStream("/book1/chapter2.html"), "chapter2.html"));
        book.addResource(
                new Resource(this.getClass().getResourceAsStream("/book1/flowers_320x240.jpg"), "flowers.jpg"));
        book.addSection(chapter2, "Chapter 2 section 1",
                new Resource(this.getClass().getResourceAsStream("/book1/chapter2_1.html"), "chapter2_1.html"));
        book.addSection("Chapter 3",
...
//            Book book = new Book();
//
//            Resource coverImg = new Resource(new FileInputStream(ResourceBundle.getBundle("info.pxdev.pfi.webclient
// .resources.Config").getString("COVER_DIR")+dto.getCoverFileName()),dto.getCoverFileName());
//
//            book.getMetadata().addTitle(dto.getTitle());
//
//            if(dto.getIdentifier().getType().getName().equals("ISBN"))
//                book.getMetadata().addIdentifier(new Identifier(Identifier.Scheme.ISBN, dto.getIdentifier()
// .getIdentifier()));
//            else
//                book.getMetadata().addIdentifier(new Identifier(Identifier.Scheme.UUID, dto.getIdentifier()
// .getIdentifier()));
//
...
//                Resource aux = new Resource(HTMLGenerator.generateChapterHtmlStream(dto,chapter), "chapter"+chapter
// .getNumber()+".html");
//                book.addSection(chapter.getTitle(), aux );
//            }
//
//            EpubWriter writer = new EpubWriter();
//            FileOutputStream output = new FileOutputStream(ResourceBundle.getBundle("info.pxdev.pfi.webclient
// .resources.Config").getString("HTML_CHAPTERS")+dto.getId_book()+"\\test.epub");
//
//            try {
