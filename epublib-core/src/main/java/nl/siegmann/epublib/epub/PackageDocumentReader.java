...
public class PackageDocumentReader extends PackageDocumentBase {
    ...
    public static void read(Resource packageResource, EpubReader epubReader, Book book, Resources resources) throws
...
//    }
    ...
    public static void readManifest(Document packageDocument, Resources resources, Book book,
                                    Map<String, String> idMapping) {
        Element manifestElement = DOMUtil.getFirstElementByTagNameNS(packageDocument.getDocumentElement(),
        ...
        for (int i = 0; i < itemElements.getLength(); i++) {
            ...
            ManifestItemReference manifestItem = new ManifestItemReference(resource,
                    ManifestItemProperties.findProperties(properties));
    ...
    private static void readGuide(Document packageDocument,
        ...
        Element guideElement = DOMUtil.getFirstElementByTagNameNS(packageDocument.getDocumentElement(), NAMESPACE_OPF,
                OPFTags.guide);
     ...
     * <p>
     * Example: If the packageHref is "OEBPS/content.opf" then a resource href like "OEBPS/foo/bar.html" will be turned
     * into "foo/bar.html"
     * @param packageHref
    ...
    private static Spine readSpine(Document packageDocument, EpubReader epubReader, Resources resources,
        ...
        Element spineElement = DOMUtil.getFirstElementByTagNameNS(packageDocument.getDocumentElement(), NAMESPACE_OPF,
                OPFTags.spine);
        ...
        result.setDirection(PageProgressionDirection
                .findDirection(spineElement.getAttribute(OPFAttributes.pageProgressionDirection)));
    ...
    public static Bindings readBindings(Document packageDocument) {
        ...
        Element bindingsElement = DOMUtil.getFirstElementByTagNameNS(packageDocument.getDocumentElement(),
                NAMESPACE_OPF, OPFTags.bindings);
    ...
    /**
     * Creates a spine out of all resources in the resources. The generated spine consists of all XHTML pages in order
     * of their href.
     * @param resources
     ...
     * <p>
     * Here we try several ways of finding this table of contents resource. We try the given attribute value, some
     * often-used ones and finally look through all resources for the first resource with the table of contents
     * mimetype.
     * @param spineElement
     * @param resources
...
//        }
    ...
    /**
     * Find all resources that have something to do with the coverpage and the cover image. Search the meta tags and the
     * guide references
     * @param packageDocument
    ...
    /**
     * Finds the cover resource in the packageDocument and adds it to the book if found. Keeps the cover resource in the
     * resources map
     * @param packageDocument
     * @param book
     */
