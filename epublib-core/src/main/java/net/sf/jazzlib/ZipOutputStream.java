...
/**
 * This is a FilterOutputStream that writes the files into a zip archive one after another. It has a special method to
 * start a new zip entry. The zip entries contains information about the file name size, compressed size, CRC, etc.
 * <p>
...
public class ZipOutputStream extends DeflaterOutputStream implements
    ...
    /**
     * Starts a new Zip entry. It automatically closes the previous entry if present. If the compression method is
     * stored, the entry must have a valid size and crc, otherwise all elements (except name) are optional, but must be
     * correct if present. If the time is not set in the entry, the current time is used.
